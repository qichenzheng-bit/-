...
def restore_questions(
    payload: dict,   # 接收任意 JSON，不做 Pydantic 校验
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    # 兼容两种格式：直接数组 或 { question_ids: [...] }
    if isinstance(payload, list):
        ids = payload
    elif isinstance(payload, dict) and "question_ids" in payload:
        ids = payload["question_ids"]
    else:
        raise HTTPException(status_code=400, detail="请求格式错误，需要 question_ids 数组")
